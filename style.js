...
const OVERDUE_LIMIT = 2 * 24 * 60 * 60 * 1000; // 2 days
...
issuedBy: loggedInUser, // ✅ ADD THIS
...
borrowList
.filter(item => !item.returned && (now - item.timestamp > OVERDUE_LIMIT))
.forEach(item => {
tbody.innerHTML += `
<tr>
<td>${item.name}</td>
